...
class NotificationService {
  ...
  async sendAppointmentCancelledNotifications(appointmentData, patientData, doctorData) {
    console.log('[NOTIFICATION SERVICE] Starting appointment cancelled notifications');
    ...
    try {
      ...
      const cancelledNotificationData = {
        ...
        time: appointmentData.time,
        appointmentId: appointmentData._id,
        reason: appointmentData.reason || 'Cancelled by doctor'
      };

      const patientPhone = String(patientData.phone || '').trim();

      if (patientData.email) {
        const template = notificationTemplates.getAppointmentCancelledTemplate(cancelledNotificationData);
        const patientEmailResult = await this.emailService.sendEmail(
          patientData.email,
          template.email.subject,
          template.email.html,
      ...
      logger.error('Failed to send appointment cancelled notifications:', error);
      return {
        success: false,
        error: error.message,
        appointmentId: appointmentData._id
